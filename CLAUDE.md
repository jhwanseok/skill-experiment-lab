...
5. 3번처럼 명시적 `Skill` 호출을 쓰는 경우, **그 호출로 로드되는 SKILL.md 자체가 ON 조건에만 붙는 고정 토큰 비용**이 된다(ponytail은 약 1,500~2,000토큰 추정). `subagent_tokens`를 주 지표로 쓸 계획이라면, 설계 단계에서 OFF 조건에도 비슷한 크기의 무관한 "더미 스킬"을 로드시키는 대조군을 추가해 이 고정비를 통제할지 미리 정한다(ponytail 실험은 이를 사전에 하지 못해 사후에 파일 크기로 역산해야 했다 — `experiments/results/ponytail-report.md`의 "토큰 차이 분해" 참고).
